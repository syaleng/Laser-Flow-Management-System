...
export function DesignOrdersPage() {
  ...
  return (
    <section dir="rtl" className="orders-page text-right">
      ...
      {orders.data && orders.data.data.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="data-table w-full min-w-[860px] text-right text-sm">
              <thead className="border-b bg-slate-50 text-xs text-slate-500"><tr><th className="px-5 py-4">د فرمایش شمېره</th><th className="px-5 py-4">مشتري</th><th className="px-5 py-4">د ډایانو شمېر</th><th className="px-5 py-4">ټول مقدار</th><th className="px-5 py-4">پاتې حساب</th><th className="px-5 py-4">د سپارلو نېټه</th><th className="px-5 py-4">حالت</th><th className="px-5 py-4">کار</th></tr></thead>
              <tbody className="divide-y divide-slate-100">{orders.data.data.map((order) => { const outstanding = Number(order.remaining_amount) > 0 && order.status !== "CANCELLED"; const editable = order.status !== "DELIVERED" && order.status !== "CANCELLED"; return <tr key={order.id} className="hover:bg-slate-50"><td className="px-5 py-4"><p className="font-semibold text-slate-900">{order.design_name}</p><p className="text-xs text-slate-500">{order.order_number}</p></td><td className="px-5 py-4 text-slate-700">{order.customer.full_name}</td><td className="px-5 py-4 font-semibold">{order.material_quantity.toLocaleString()}</td><td dir="ltr" className="px-5 py-4 text-right font-semibold">{money(order.total_amount)}</td><td dir="ltr" className={`px-5 py-4 text-right font-bold ${outstanding ? "text-amber-700" : "text-emerald-700"}`}>{outstanding ? money(order.remaining_amount) : "—"}</td><td className="px-5 py-4">{order.expected_delivery_date}</td><td className="px-5 py-4"><DesignOrderStatusBadge status={order.status} /></td><td className="px-5 py-4"><div className="flex items-center gap-2"><Link className="rounded-lg bg-brand-50 px-3 py-2 font-bold text-brand-700 hover:bg-brand-100" to={`/design-orders/${order.id}`}>کتل</Link>{editable && <Link className="rounded-lg border border-slate-200 px-3 py-2 font-bold text-slate-700 hover:bg-slate-50" to={`/design-orders/${order.id}/edit`}>اصلاح</Link>}<Link className="rounded-lg border border-blue-100 px-3 py-2 text-xs font-bold text-blue-700 hover:bg-blue-50" to={`/customers/${order.customer.id}/edit`}>مشتری</Link></div></td></tr>; })}</tbody>
            </table>
          </div>
